...
import { englishData, spanishData } from './cardData';
...
export const Card = ({ shelter, language }: CardProps) => {
  ...
  const [cardData, setCardData] = useState(spanishData);

  useEffect(() => {
    switch (language) {
      case 'English':
          setCardData(englishData);
          break;
    
      default:
          setCardData(spanishData);
          break;
    }
    ...
    <div className='max-w-sm bg-white w-max my-6 rounded-lg transition-transform hover:scale-105'>
      ...
      <div className='p-5 bg-white rounded-b-md'>
        ...
        <div className='flex flex-col items-center'>
          <a href={shelter.web} className='inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-violet-800 rounded-lg hover:bg-violet-600 focus:ring-4 focus:outline-none focus:bg-violet-600'>
            { cardData.btnText }
